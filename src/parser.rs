...
impl Tag {
    ...
    /// Return the resolved tag handle or prefix.
    #[must_use]
    pub fn handle(&self) -> &str {
        &self.handle
    }

    /// Return the suffix following the resolved tag handle or prefix.
    #[must_use]
    pub fn suffix(&self) -> &str {
        &self.suffix
    }

    /// Return the tag handle as written before `%TAG` directive resolution.
    #[must_use]
    pub fn original_handle(&self) -> &str {
...
impl<'input, T: BorrowedInput<'input>> Parser<'input, T> {
        ...
        'input: 'a,
    {
        ...
        match token {
            QueuedTokenType::Comment(comment) => {
                let placement = self.refined_comment_placement(span, comment.placement());
                Ok(Some((Event::Comment(comment.into_text(), placement), span)))
            }
    ...
    {
        match *self.peek_token()? {
            QueuedToken(span, QueuedTokenType::StreamStart) => {
...
impl<'input, T: BorrowedInput<'input>> core::iter::FusedIterator for Parser<'input, T> {}
